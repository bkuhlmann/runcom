...
RSpec.describe Runcom::Paths::Combined do
  subject(:combined) { described_class.new home, directories }

  let :home do
    Runcom::Paths::Standard.new Runcom::Pair.new("TEST_HOME", "/one"), environment
  end

  let :directories do
    Runcom::Paths::Directory.new Runcom::Pair.new("TEST_DIRS", "/two:/three"), environment
  end
  ...
  describe "#all" do
    ...
    context "with set environment" do
      let :home do
        Runcom::Paths::Standard.new Runcom::Pair.new("TEST_HOME", nil), environment
      end

      let :directories do
        Runcom::Paths::Directory.new Runcom::Pair.new("TEST_DIRS", nil), environment
      end
